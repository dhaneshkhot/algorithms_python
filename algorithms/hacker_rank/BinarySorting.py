class BinarySorting:
    def rearrange(self, elements):
        ...
        for i in elements:
            binary_values.append("{:04b}".format(i))

        for i in range(arr_len):
            for j in range(0, arr_len-i-1):
                if get_no_of_ones(binary_values[j]) > get_no_of_ones(binary_values[j + 1]):
                    # tmp = binary_values[j]
                    # binary_values[j] = binary_values[j+1]
                    # binary_values[j + 1] = tmp

                    binary_values[j], binary_values[j+1] = binary_values[j+1], binary_values[j]

                    # or_tmp = original_arr[j]
                    # original_arr[j] = original_arr[j+1]
                    # original_arr[j+1] = or_tmp

                    original_arr[j], original_arr[j+1] = original_arr[j+1], original_arr[j]

                elif (get_no_of_ones(binary_values[j]) == get_no_of_ones(binary_values[j + 1])) and elements[j] > elements[j+1]:
